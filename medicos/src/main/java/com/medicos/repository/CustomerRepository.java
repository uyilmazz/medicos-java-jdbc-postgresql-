...
public class CustomerRepository extends BaseRepository<Customer> {
	...
	public Customer login(CustomerLoginDto customerLoginDto) throws SQLException {
		Customer customer = null;
		String sql = "Select * from customers where email = ? and password = ?";
		connect();
		PreparedStatement statement = connection.prepareStatement(sql);
		statement.setString(1, customerLoginDto.getEmail());
		statement.setString(2, customerLoginDto.getPassword());
		ResultSet resultSet = statement.executeQuery();
		if(resultSet.next()) {
			customer = parse(resultSet);
		}
	...
	protected Customer parse(ResultSet resultSet) throws SQLException {
		...
		return new Customer(id, firstName, lastName, email, password, imageUrl);
	}
}
