...
public class ProductRepository extends BaseRepository<Product> {
	...
	public List<Product> findAllByCategoryId(int categoryId) throws SQLException {
		String condition = String.format("where p.category_id = %s", categoryId);
		String sql = String.format("%s %s", SELECT_STRING, condition);
		return super.findAll(sql);
	}

	public Product findById(long id) throws SQLException {
		String sql = String.format("%s %s", SELECT_STRING, "where p.id = ?");
		return super.findById(sql, id);
	}

	public Product findByName(String name) throws SQLException {
		Product product = null;
		String sql = String.format("%s %s", SELECT_STRING, "where p.name = ?");
		connect();
	...
	protected Product parse(ResultSet resultSet) throws SQLException {
		...
		Product product = new Product(id, name, description, howToUse, salesPrice, imageUrl);
