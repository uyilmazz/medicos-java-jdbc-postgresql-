...
@WebServlet("/api/departments/findById")
public class DepartmentFindByIdServlet extends HttpServlet{
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if(isLong) {
				...
				DataResult<Department> result = departmentService.getById(id);
				if(result.isSuccess()) {
					document = DepartmentXml.format(result.getData());
				}else {
